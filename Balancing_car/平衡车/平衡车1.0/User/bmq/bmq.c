...
void Encoder_Init_TIM4(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;        //端口配置 PA0 PA1
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //浮空输入
  GPIO_Init(GPIOB, &GPIO_InitStructure);                                              //根据设定参数初始化GPIOA

  NVIC_InitStruct.NVIC_IRQChannel = TIM4_IRQn;  //定时器2中断
  ...
  TIM_TimeBaseStructure.TIM_Prescaler = 144; // 预分频器
...
void Encoder_Init_TIM3(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_5;        //端口配置 PA6 PA7
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //浮空输入
  GPIO_Init(GPIOB, &GPIO_InitStructure);                                              //根据设定参数初始化GPIOA  
