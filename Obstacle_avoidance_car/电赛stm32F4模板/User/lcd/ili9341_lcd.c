...
void ILI9341_DispChar_CH ( uint16_t usX, uint16_t usY, uint16_t usChar )
{
  ...
  GetGBKCode ( ucBuffer, usChar );	
    ...
    /* 取出两个字节的数据，在lcd上即是一个汉字的一行 */
		...
		printf("0X%X,0X%X,0X%X,0X%X,",ucBuffer [ rowCount * 4 ],ucBuffer [ rowCount * 4 +1],ucBuffer [ rowCount * 4+2 ],ucBuffer [ rowCount * 4 +3]);
									...
									uint8_t en_cn)		//0为英文，1为中文	
{
  ...
  unsigned int yrIntFloat_16=(in_heig<<16)/out_heig+1;
	...
	//检查参数是否合法
	if(in_width >= 48) return;												//字库不允许超过32像素
	if(in_width * in_heig == 0) return;	
	if(in_width * in_heig >= 48*48 ) return; 					//限制输入最大 32*32
...
void picture_show(uint16_t x,uint16_t y,char* picture)
{	
	uint16_t x1=0,y1=0;long i=0;uint16_t date;
	ILI9341_FillColor ( 320*240,BLACK);
	for(;y1<y;y1++)
	{
	    for(x1=0;x1<x;x1++)
		{
			date=(picture[i++]<<8)+picture[i++];
			ILI9341_SetCursor (x1+((240-x)/2), y1+((320-y)/2));
			ILI9341_Write_Cmd ( CMD_SetPixel );			
			ILI9341_Write_Data ( date );			
		}
