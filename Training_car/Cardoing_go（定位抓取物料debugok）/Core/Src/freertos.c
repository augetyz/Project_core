...
void doing_Task(void const * argument)
{
  ...
  /* Infinite loop */
    ...
    static uint8_t Task_select=Start,Task_verify=Start,color_sign=0;//执行调度参数
...
//      imu_date.IMU[2]=0;
      ...
      switch(Task_select)             
      {
          ...
          case To_QR_Code:
              ...
              {
                  vTaskSuspendAll();
                  Task_select=Task_verify=At_QR_Code; 
                  V831_QuickMark_do();
                  V831_1_status=0;
                  xTaskResumeAll();                  
              }
          ...
          case At_QR_Code://扫描二维码
              ...
              else
              {
                  ...
                  else
                  {
                      ...
                      V831_die();
                      Task_select=Task_verify=To_RMA; 
                      V831_1_status=0;
          ...
          case To_RMA:
              ...
              {
                  vTaskSuspendAll();
                  Task_select=Task_verify=At_RMA;
                  V831_1_status=0;
                  V831_die();
                  xTaskResumeAll();                  
              }
        ...
        case At_Turn_1_Find_Line:
            ...
            else
            {
                ...
                else
                {
                    if (Do_capture_sign == 0)
                    {
                        if ((black_line_date.date[0] == 0XFF && black_line_date.date[1] == 0XFF) || black_line_date.date[1] < -5 || black_line_date.date[1] > 5) // 没有检测到黑线
                        {
                            // 以80的速度前进
                            speed_CTRL(80, 80, 80, 80); // 前进
                        }
                        else
                            Do_capture_sign++; // 开始捕获
                    }
        ...
        case To_Put_down_1:
            ...
            if (crosswise_angle_distance(angle_adjust, imu_date, distance_now, -800) == 1) // 放下1
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        V831_2_color_circle_do();
                        servo_all_move(servo_motion[23]);
                        osDelay(servo_motion[23].time);
                        servo_all_move(servo_motion[31]);
                        osDelay(servo_motion[31].time);
                        vTaskSuspendAll();
                        Task_select = Task_verify = At_Put_down_1_2;
