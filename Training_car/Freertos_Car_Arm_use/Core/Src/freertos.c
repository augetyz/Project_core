...
extern servo_status Servo;

uint8_t OS_status = 0;
uint8_t debug_date[600] = {0};
uint8_t IMU_date[IMU_speed] = {0};
uint8_t Servo_debug_date[30];
uint8_t biu;
...
void DMA_transfer_Rest(void)
{
    HAL_UART_DMAStop(&huart6);
    HAL_UART_Receive_DMA(&huart6,Servo_debug_date,sizeof(Servo_debug_date));
}
  ...
  for (;;)
  {
    if ((GPIOA->IDR & (1 << 15)) == 0)
    {
      ...
      if ((GPIOA->IDR & (1 << 15)) == 0)
      {
        ...
        /*do something*/
//        printf("按键触发\n");
        HAL_UART_Transmit_DMA(&huart1, Servo_debug_date, 30);
        osDelay(100);
        printf("\n%d \n",__HAL_DMA_GET_COUNTER(&hdma_usart6_rx));
        
        if(__HAL_DMA_GET_COUNTER(&hdma_usart6_rx)<1)
        {
            DMA_transfer_Rest();
            printf("\n%d\n",__HAL_DMA_GET_COUNTER(&hdma_usart6_rx));
        }  
...
void servo_Task(void const * argument)
{
  ...
  for (;;)
  {
     if(__HAL_DMA_GET_COUNTER(&hdma_usart6_rx)==10||__HAL_DMA_GET_COUNTER(&hdma_usart6_rx)==20||__HAL_DMA_GET_COUNTER(&hdma_usart6_rx)==0||__HAL_DMA_GET_COUNTER(&hdma_usart6_rx)==5)
     {
         switch(servo_date_deal(Servo_debug_date))
         {
             case 0:printf("error1\n");DMA_transfer_Rest();clearArray(Servo_debug_date,30);
                break;
             case 1:DMA_transfer_Rest();servo_Ctrl(&Servo);printf("biu1\n");
                break;
             case 2:DMA_transfer_Rest();servo_Ctrl(&Servo);printf("biu2\n");
                break;
             case 3:printf("error2\n");DMA_transfer_Rest();clearArray(Servo_debug_date,30);
                break;
         }
     }
     else
     {
         osDelay(2);
         if(__HAL_DMA_GET_COUNTER(&hdma_usart6_rx)%5!=0)
         {
             DMA_transfer_Rest();
         }
     }
