...
void doing_Task(void const *argument)
{
    ...
    for (;;)
    {
        ...
        {
        case Start:
            if (crosswise_angle_distance(0, imu_date, distance_now, -180) == 1) // 离开出发点左移28cm
            {
                speed_CTRL(0, 0, 0, 0);
        ...
        case To_QR_Code:
            if (advance_angle_distance(0, imu_date, distance_now, 480) == 1) // 到达二维码扫描区
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
        ...
        case To_RMA:
            if (advance_angle_distance(0, imu_date, distance_now, 580) == 1) // 抬起1
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
                Task_select = At_RMA;
