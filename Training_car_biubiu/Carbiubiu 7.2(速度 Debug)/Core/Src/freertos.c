...
QueueHandle_t debug_Speed_Queue = NULL;
...
void MX_FREERTOS_Init(void)
{
    ...
    debug_Speed_Queue = xQueueCreate((UBaseType_t)1, /* 消息队列的长度 */ // 实时速度参数传递消息队列
        (UBaseType_t)sizeof(biu));           /* 消息的大小 */   
...
//    float serve_angle[5]={0,0,0,0,0};
    ...
    for (;;)
    {
        xQueueReceive(debug_Speed_Queue, &speed_now, 15);
...
//        FSUS_QueryServoAngle(servoUsart,6,&serve_angle[4]);
        ...
        sprintf((char*)debug_date, "%d,%d,%d,%d,%d,%d,%d,%d\n",
...
void speed_Task(void const* argument)
{
    ...
    for (;;)
    {
        ...
        xQueueSend(debug_Speed_Queue, &speed, 0); /* 等待时间 0 */
...
biu_int_16 speed_debug_use={0,0,0,0};
...
//                servo_all_move(servo_motion[26]);
        ...
        case Color_cicle_location:  // 判断颜色圈位置
            ...
            break;  // 跳出循环
        case End:
            speed_CTRL(speed_debug_use.date[0],speed_debug_use.date[1],speed_debug_use.date[2],speed_debug_use.date[3]);   
//            vTaskSuspend(myTask_pidHandle);
//            speed_ctrl(Motor1, speed_debug_use.date[0]);
//            speed_ctrl(Motor2, speed_debug_use.date[1]);
//            speed_ctrl(Motor3, speed_debug_use.date[2]);
//            speed_ctrl(Motor4, speed_debug_use.date[3]);
//            vTaskSuspend(NULL);
            break;
        }
