...
//    uint8_t IDR_X_flag = 0; // IDR_X坐标标志位
    ...
    for (;;)
    {
        ...
        case target_location:
            ...
            if ( (int16_t)(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) < -1 ||(int16_t) (IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 1 )
            {
            ...
            else if ((int16_t)(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) < -1 || (int16_t)(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 1)
            {
...
void target(imu imu_date, IDR_date IDR_date_use,IDR_date IDR_date_goal, float angle_standard)
{
    angle_standard = imu_date.IMU[2];
    static int back_sign = 0, IDR_speed_sum1 = 0, IDR_speed_dert = 0, IDR_speed_sum2 = 0, IDR_speed_use = 0;
    static uint8_t target_location_sign = 0;
    if (abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 2)
    {
        ...
        IDR_speed_use = IDR_speed_dert * 3 + IDR_speed_sum2 * 0.04;
        IDR_speed_use = IDR_speed_use > 1000 ? 1000 : (IDR_speed_use < -1000 ? -1000 : IDR_speed_use);
    ...
    else if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 2)
    {
        ...
        IDR_speed_sum1 += IDR_speed_dert;
        IDR_speed_sum1 = IDR_speed_sum1 > 2000 ? 2000 : (IDR_speed_sum1 < -2000 ? -2000 : IDR_speed_sum1);
        IDR_speed_use = IDR_speed_dert * 3 + IDR_speed_sum1 * 0.04;
        IDR_speed_use = IDR_speed_use > 1000 ? 1000 : (IDR_speed_use < -1000 ? -1000 : IDR_speed_use);
