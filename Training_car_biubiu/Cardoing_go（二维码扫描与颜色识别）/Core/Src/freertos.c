...
void usart_Task(void const * argument)
{
  ...
  /* Infinite loop */
    ...
    HAL_UART_Receive_DMA(&huart3, usart_date, 22);   
  for (;;)
  {
      ...
      for(i=0;i<8;i++)
      {
          if(usart_date[i]==0X2C&&usart_date[i+10]==0X5B)
          {
              num=i;
              if(usart_date[num+1]==0X12)
                sign=1;
              else if(usart_date[num+1]==0X21)
                sign=2;
              else
                sign=0;
              break;              
      ...
      xTaskResumeAll();
//      HAL_UART_Transmit_DMA(&huart1, usart_date, 14);
      if(sign==1)
      {
          
          for(i=0;i<2;i++)
          {
              *(f+i)=*(usart_date+num+i+2);
          }
//          i=*(f);*(f)=*(f+1);*(f+1)=i;
          
          for(i=0;i<2;i++)
          {
              *(h+i)=*(usart_date+num+i+4);
          }
//          i=*(h);*(h)=*(h+1);*(h+1)=i;
          if((x!=520&&y!=520)&&(y!=9)) 
          {
...
//              xQueueSend(color_Queue,&color_date,0);
      ...
      {
          usart_date[num+6]=usart_date[num+7];
          usart_date[num+7]=usart_date[num+8];
          usart_date[num+8]=usart_date[num+9];
          x=0;
          for(i=0;i<6;i++)   
          {
              if(usart_date[num+3+i]=='1')
                  x|=0X01<<(5-i)*2;
              else if(usart_date[num+3+i]=='2')
                  x|=0X02<<(5-i)*2;
              else if(usart_date[num+3+i]=='3')
                  x|=0X03<<(5-i)*2;
              else
              { 
                x=0;
...
//      HAL_UART_Transmit_DMA(&huart1, usart_date, 22);          
      }
    HAL_UART_Receive_DMA(&huart3, usart_date, 22);    
    osDelay(10);
  }
...
void doing_Task(void const * argument)
{
  ...
  /* Infinite loop */
    ...
    biu distance_now;
    biu_int_16 color_date;
    static servo_status Servo={800,550,2200,1900,1900,1500,2000,1};
    
    static uint8_t i=Start;//执行调度参数
    
    uint8_t servo_status,V831_status=0,sign=0;
    int16_t color=0;//颜色顺序存储
//    xQueueSend(servo_Queue,&Servo,0);
  ...
  for (;;)
  {
      ...
      {
          case Start:
              if(crosswise_angle_distance(0,imu_date,distance_now,-250)==1)//离开出发点
                  i=Start_Calibration;
              break;  
          ...
          case To_QR_Code:
              if(advance_angle_distance(0,imu_date,distance_now,700)==1)//到达二维码扫描区
              {
                  i=At_QR_Code; 
                  V831_QuickMark_do();
              }
          ...
          case At_QR_Code:
              ...
              {
                  color=color_date.date[3];  
                  printf("biu:%d,%d,%d,%d\n",color_date.date[0],color_date.date[1],color_date.date[2],color_date.date[3]);                
                  V831_die();
                  i=To_Take_up; 
                  V831_status=0;
              }                              
